...
# ==========================================
# ⚙️ CONFIGURATION (API KEY REMOVED)
# ==========================================
API_ID = 34179263
API_HASH = "6a54e9a94340b6a6983df4b7acfccd00"
SESSION_NAME = 'new_osint_session'  
TARGET_BOT = 'utkarsh_hackerr_bot'  # 🔴 Direct aapke bot par jayega
DB_NAME = 'anurag_data.db'

# ==========================================
# 🧹 SMART TEXT CLEANER (REMOVES JUNK ADS)
# ==========================================
def clean_bot_data(raw_text):
    """Sirf ads aur faltu links hatayega, baki data safe rakhega"""
...
# ==========================================
# 🎨 UI CODE (CYBER-GLASS + MADE BY UTKARSH)
# ==========================================
HTML_CODE = """
<!DOCTYPE html>
<html lang="en">
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .brand-icon {
            ...
            font-size: 20px; box-shadow: 0 0 15px rgba(14, 165, 233, 0.4);
        }

        .nav-label {
            font-size: 11px; font-weight: 600; color: #64748b;
            text-transform: uppercase; letter-spacing: 2px; margin-bottom: 15px; padding-left: 5px;
        }
        ...
        .buy-tools-btn {
            ...
            box-shadow: 0 8px 20px rgba(16, 185, 129, 0.25); transition: 0.3s;
        ...
        .buy-tools-btn:hover::after { left: 150%; }
        .buy-tools-btn:hover { transform: translateY(-3px); box-shadow: 0 12px 25px rgba(16, 185, 129, 0.4); }
        ...
        .page-desc { color: #94a3b8; font-size: 15px; margin-top: 8px; font-weight: 300; }

        .search-wrapper {
            background: rgba(0, 0, 0, 0.3); padding: 8px; border-radius: 20px;
        ...
        .search-wrapper:focus-within {
            ...
            box-shadow: 0 0 20px rgba(56, 189, 248, 0.15), inset 0 2px 4px rgba(0,0,0,0.5);
        }
        .search-input {
            flex: 1; border: none; background: transparent; padding: 15px 25px;
            font-size: 16px; font-weight: 500; color: #fff; outline: none;
        ...
        .search-btn {
            background: #e2e8f0; color: #0f172a; border: none; padding: 0 35px;
        ...
        .results-container {
            flex: 1; background: rgba(2, 6, 23, 0.4); border-radius: 20px;
        ...
        .result-title { font-size: 13px; font-weight: 600; color: #94a3b8; text-transform: uppercase; letter-spacing: 1px; }
        ...
        .data-text {
            ...
            line-height: 1.8; color: #38bdf8; white-space: pre-wrap; word-wrap: break-word;
        ...
        .error-text { 
            ...
            text-shadow: 0 0 10px rgba(244, 63, 94, 0.2) !important; 
            font-weight: 500; 
            font-family: 'Outfit', sans-serif;
        }
        ...
        .spinner {
            ...
            border-top: 3px solid #38bdf8; border-right: 3px solid #8b5cf6; border-radius: 50%;
            animation: spin 0.8s linear infinite; margin: 0 auto 20px auto;
        }
        .scan-text {
            font-family: 'Fira Code', monospace; color: #38bdf8; font-size: 14px;
...
<body>
    ...
    <div class="dashboard">
        <div class="sidebar">
            ...
            <button class="nav-btn active" id="btn-live" onclick="setMode('live')">
                <span style="font-size: 18px;">📡</span> Live Node Scan
            </button>
            <button class="nav-btn" id="btn-db" onclick="setMode('db')">
                <span style="font-size: 18px;">🗄️</span> Secure Vault
            </button>

            <button class="buy-tools-btn" onclick="buyTools()">
                🛒 Premium Tool Access
            </button>
        </div>
        ...
        <div class="main">
            ...
            <div class="search-wrapper">
                <input type="text" id="cmdInput" class="search-input" placeholder="Enter Target Identity (Number)..." autocomplete="off">
            ...
            <div class="results-container">
                <div class="result-header">
                    ...
                    <button class="copy-btn" id="copyBtn" onclick="copyData()" style="display:none;">
                        📋 Copy Payload
                    </button>
                </div>
                ...
                <div class="result-body" id="resultBody">
                    <div class="empty-state" id="initialState">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#334155" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="margin-bottom:15px;"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>
                        <p style="font-size: 14px;">Awaiting command input parameters...</p>
                    </div>
                    
                    <div class="loader-wrapper" id="loaderState">
                        <div class="spinner"></div>
                        <div class="scan-text">ESTABLISHING CONNECTION...</div>
                    </div>

                    <div class="data-text" id="finalData" style="display:none; animation: slideUp 0.4s ease-out;"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function executeSearch() {
            ...
            document.querySelector('.scan-text').innerText = mode === 'live' ? 'BYPASSING SECURITY PROTOCOLS...' : 'SCANNING LOCAL DB...';
            ...
            .then(data => {
                ...
                if(data.found) {
                    document.getElementById('resHeaderTitle').innerText = `✅ DECRYPTION SUCCESSFUL: ${val}`;
                    ...
                    // Agar bot ne koi text diya hai (jaise "Subscription over") to wo dikhao
                    if(data.reply && data.reply !== "Data Not Found") {
                        dataBox.innerHTML = data.reply.replace(/\\n/g, '<br>');
                    } else {
                        dataBox.innerHTML = "DATA NOT FOUND!<br><br><span style='color:#94a3b8; font-size:14px;'>The record does not exist or the bot did not reply.</span>";
                    }
                }
            })
            .catch(err => {
                ...
                dataBox.style.display = 'block';
                dataBox.classList.add('error-text');
                dataBox.innerHTML = "FATAL ERROR: Connection to extraction server failed.";
            });
        }
        ...
        function buyTools() {
            window.open('https://t.me/utkarsh_hackerr_bot', '_blank');
        }
...
def run_live():
    ...
    try:
        with TelegramClient(SESSION_NAME, API_ID, API_HASH, loop=new_loop) as client:
            ...
            # ⚡ SMART DUAL-CATCH POLLING
            for i in range(40): # Max wait ~10 sec
                time.sleep(0.25)
                ...
                # Agar pehla reply mil gaya hai...
                if new_msg_found:
                    # To sirf 0.5 second rukenge taaki agar bot ka dusra reply (Asli Data) raste me ho to wo bhi aa jaye.
                    time.sleep(0.5) 
                    ...
                    for msg in msgs:
                        if not msg.out and (msg.date.timestamp() > start_time - 1):
                            ...
                            if cleaned_text and cleaned_text not in collected_messages and text.lower().strip() not in ["processing...", "wait...", "searching...", "please wait..."]:
                                collected_messages.append(cleaned_text)
                    break # Dono message uthaye aur loop tod diya (Total time < 1-2 sec)
...
async def login_system():
    ...
    if not await client.is_user_authorized():
        print("⚠️ NEW NUMBER DETECTED: LOGIN REQUIRED!")
